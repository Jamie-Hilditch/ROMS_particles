...
def create_trilinear_interpolation_kernel(
    field: str, particle_field: str | None = None, dtype: npt.DTypeLike = np.float64
) -> ParticleKernel:
...
def create_horizontal_bilinear_interpolation_kernel(
    field: str, particle_field: str | None = None, dtype: npt.DTypeLike = np.float64
) -> ParticleKernel:
    ...
    def kernel_fn(
        ...
        idx = offset_indices_2D(
            particles["yidx"][pidx], particles["xidx"][pidx], field_off
        )
